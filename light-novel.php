...
// Query to select all light novel books
$sql = "SELECT * FROM books WHERE type = 'Light Novel'";
$result = mysqli_query($conn, $sql);

// Check if there are light novel books available
if (mysqli_num_rows($result) > 0) {
    echo "<div class='manga-books-container'>"; // Opening manga-books-container
    // Output light novel books
    while ($row = mysqli_fetch_assoc($result)) {
        // Wrap the entire light novel book container in an anchor tag
    ...
    echo "<p>No light novel books available.</p>";
}
